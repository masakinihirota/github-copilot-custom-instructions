...
* プロジェクトで、新しい会話を始める時は必ずメモリーファイル `memory.md`を読み込んでください。そして前回までの過去の会話や作業内容を把握したうえで作業を再開してください。
...
現状の把握、進捗状況、残りの作業を把握するためのメモリーファイルです。
前回どこまでやったかを毎回説明し直さずにスムーズに対話を続行できるようにします。

セッションを超えてコンテキストを保持できるようにします。
会話が途切れたり、処理が中断してもすぐに前回の続きができるようにする一種のメモリーのファイルです。

_memoryフォルダを作成し、以下のファイルを作成します。

```
_memory/activeContext.md
_memory/productContext.md
_memory/progress.md
_memory/projectbrief.md
_memory/systemPatterns.md
_memory/techContext.md

```

## 概要
`_memory` フォルダは、プロジェクトの進行状況や文脈を記録するためのメモリーファイルを格納するディレクトリです。このフォルダ内のファイルは、プロジェクトの状態を正確に把握し、次回の作業をスムーズに再開するために使用されます。

## ファイル構成
以下のMarkdownファイルが含まれています。それぞれの役割を簡単に説明します。

### 1. `projectbrief.md`
- **目的**: プロジェクトの概要やゴールを記述します。
- **内容**: プロジェクトの目的、解決する問題、期待される成果、短期・長期目標。

### 2. `productContext.md`
- **目的**: プロダクトの目的やユーザーに提供する価値を記述します。
- **内容**: 解決する課題、提供する価値、プロジェクトの存在意義。

### 3. `systemPatterns.md`
- **目的**: システムアーキテクチャや設計パターンを記録します。
- **内容**: 使用するアーキテクチャ、採用するデザインパターン、コンポーネント間の関係。

### 4. `techContext.md`
- **目的**: 使用技術や開発環境を記録します。
- **内容**: フロントエンド、バックエンド、データベース、IDE、バージョン管理。

### 5. `activeContext.md`
- **目的**: 現在の作業内容や次のステップを記録します。
- **内容**: 実施中のタスク、次に取り組むべきタスク。

### 6. `progress.md`
- **目的**: 進捗状況や既知の課題を記録します。
- **内容**: 完了したタスク、未完了のタスク、既知の課題。

## 運用方法
1. **初期化**: プロジェクト開始時に各ファイルを作成し、初期情報を記述します。
2. **更新**: 作業の進行に応じて、各ファイルを随時更新します。
3. **参照**: 次回の作業開始時に、これらのファイルを参照して文脈を把握します。
